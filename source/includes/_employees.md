...
| `latest_status`    | `created` - An employee record has been created<br>`not_invited` - Invitation has not been sent<br>`invited` - An invitation has been sent to the employee<br>`onboarding` - The employee has accepted the invitation and is completing the onboarding process<br>`country_mismatch` - User's country is different than employee's country<br>`active` - employee has completed onboarding and is ready to receive payouts |
