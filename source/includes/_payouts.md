...
| `latest_status`             | `created` - The payout has been created.<br>`not_funded` - The payout is waiting to be funded. The payout is not currently visible to the recipient.<br>`onboarding_incomplete` - The payout has been funded, and the recipient has been invited to Gigapay, but onboarding is not yet complete.<br>`notification_sent` - The payout is visible in the recipient's Gigapay dashboard with its due date.<br>`available` - The payout is ready to be accepted by the recipient.<br>`complete` - The payout has been accepted by the recipient and the payment process has been finalized. Gigapay will handle any remaining administrative tasks. |
